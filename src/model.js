...
function createDemoWellbeingHistory() {
  ...
  const values = [
    [[2,3,2,3],[3,3,3,2],[4,4,4,2]],
    [[2,3,2,3],[3,3,2,3],[5,4,4,1]],
    [[3,3,3,2],[3,4,3,2],[4,4,4,1]],
    [[2,3,2,3],[3,3,3,2],[5,5,4,1]],
    [[3,4,3,2],[3,4,3,2],[4,4,4,1]],
    [[2,3,2,2],[3,4,3,2],[4,4,4,1]],
    [[3,4,3,2],[3,4,3,2],[4,4,4,1]],
  ]
  return days.flatMap((day, dayIndex) => ['morning','day','evening'].map((type, typeIndex) => {
    ...
    return {
      timestamp: `2026-08-${String(day).padStart(2,'0')}T${String(hour).padStart(2,'0')}:00:00+03:00`,
      day,
      type,
      energy,
      mood,
      focus,
      anxiety,
      sleepHours: type === 'morning' ? [6.5,6,7,6.5,7.5,7,7][dayIndex] : null,
      sleepQuality: type === 'morning' ? [3,3,4,3,4,4,4][dayIndex] : null,
      demo: true,
    }
  }))
}
...
export function calculateWellbeingStats(entries) {
  if (!entries.length) return { sampleSize: 0, averageEnergy: null, averageMood: null, averageFocus: null, averageAnxiety: null, averageSleep: null, peakEnergyType: null }
  ...
  return {
    ...
    averageEnergy: roundedAverage(entries,'energy'),
    averageMood: roundedAverage(entries,'mood'),
    averageFocus: roundedAverage(entries,'focus'),
    averageAnxiety: roundedAverage(entries,'anxiety'),
    averageSleep: roundedAverage(entries,'sleepHours'),
    averageSleepQuality: roundedAverage(entries,'sleepQuality'),
    peakEnergyType: energyByType[0]?.type ?? null,
  }
}
...
export function addWellbeingEntry(state, values, date = new Date()) {
  ...
  const entry = {
    ...
    day: Number(new Intl.DateTimeFormat('en-GB', { timeZone: 'Europe/Moscow', day: '2-digit' }).format(date)),
